...
### Fixed
- Windows: the full test suite now passes on `windows-latest`. Escaped git-root path components (workspace, project-state, and log names) are capped at 64 chars with a hash suffix so deep repo paths no longer trip git's `'$GIT_DIR' too big` limit (existing short names are unchanged; legacy long names still resolve). Agent `prompt.file` paths get the same slash-only validation as routine artifact paths (POSIX-absolute and backslash paths rejected on every platform). Worker prompts render all paths — including the workspace directory — in forward-slash form, matching what git itself prints on Windows.
