...
- Set `consumes:` (a list of task-relative paths) to render a `## Inputs` block into the worker prompt — each path is existence-checked, and missing ones are marked so the worker knows an expected input is absent. `produces:` (a single filename) names the artifact a step writes and drives `branches:` predicates. Both are valid only on regular steps.
