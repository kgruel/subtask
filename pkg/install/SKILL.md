...
- `subtask wait <task>...` — block until named tasks finish (completion barrier). Defaults to all named tasks; `--any` returns on the first, `-n N` on the Nth, `--timeout` bounds the wait. Read-only: it never mutates task state. Exit 0 satisfied, 2 satisfied-with-a-worker-error, 3 timeout. See gotcha 5 for the fan-out pattern.
