...
`subtask draft` also accepts `--follow-up <task>` to carry context forward from a prior task (useful for chained work; rarely needed). For any follow-up with readable parent files, the child always gets the parent's artifacts (TASK.md/PLAN.md/PROGRESS.json and any produced files) injected as a read-only `## Parent Context` block — durable refs beat implicit conversation. When the parent's session can also be resumed, that resumption rides alongside the artifacts. Relatedly, `subtask send --pinned-base` makes the first send branch from the draft-time base commit instead of current base-branch HEAD — use it when drafting a batch up front against a base that will keep moving (e.g. staged refactors merging sequentially). The fallback aspect is only that continuity survives when resume is impossible: if the parent was merged/closed, was never dispatched, or ran a different adapter, the artifacts alone carry it forward and the follow-up never fails for lack of a resumable session. Under `send --detach` this continuity-downgrade warning goes to the task's supervisor log instead of the terminal, and under `-q` it is suppressed entirely (it lands in no log). The durable signal in every case is the `follow_up_artifact_only: true` stamp on the `worker.started` event — read the task's `history.jsonl` directly for it, since the rendered `subtask log` / `subtask trace` output does not surface that field.
...
- `subtask wait <task>...` — block until named tasks finish (completion barrier). Defaults to all named tasks; `--any` returns on the first, `-n N` on the Nth, `--timeout` bounds the wait; `--json` emits a per-task result array on completion. Read-only: it never mutates task state. Exit 0 satisfied, 2 satisfied-with-a-worker-error, 3 timeout. See gotcha 5 for the fan-out pattern.
- `subtask workspace <task>` — print the git worktree path.
- `subtask ask "..."` — one-off question with no task, runs in cwd. Passthrough to the configured adapter; useful for quick lookups (`--agent` to override). Not a task primitive. `--follow-up <task>` continues from a prior ask/task's context — but it does not carry the earlier invocation's `--agent`: re-pass `--agent` on every follow-up or it falls back to config.
...
5. **`send` blocks; `stage` sometimes blocks.** `subtask send` is synchronous — run it with `run_in_background: true` on the Bash tool call, or use `subtask send --detach` (below). Do not add `&` to the command itself; that backgrounds the subprocess in the shell and the Bash tool exits immediately (exit 0), which defeats the notification — you'll think the send is done when the worker hasn't even started. Don't pipe it through `tail`/`head`; use `-q` for quiet output and `subtask reply` to fetch the canonical reply from history. `subtask stage` also blocks when the target step auto-dispatches (see Routines for the triggers); pass `--no-send` to stay passive.

   **Fan-out barrier.** To dispatch a batch and wait on all of it: `subtask send --detach <task> "..."` is the native non-blocking dispatch — it returns as soon as a detached supervisor process has claimed the task (its own process outlives your shell), so you can fire a batch and then `subtask wait <task-a> <task-b> ...` to block until the whole batch finishes (`--any`/`-n N` for partial barriers, `--timeout` to bound it). Retrieve each reply with `subtask reply <task>` after it completes. `wait` is read-only and works headless. Fallback: if you'd rather not detach, run each `subtask send <task> "..."` with `run_in_background: true` and `wait` on them the same way.
