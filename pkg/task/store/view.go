...
func BuildViewFromTaskView(tv TaskView) (*task.View, error) {
	...
	if len(tv.ProgressSteps) > 0 {
		...
		for i, s := range tv.ProgressSteps {
			v.ProgressSteps[i] = s
		}
	}
...
// convertStepsForDiagram converts []task.StepView to []render.DiagramStep for
...
func convertStepsForDiagram(steps []task.StepView) []render.DiagramStep {
	if len(steps) == 0 {
		return nil
	}
